...
import com.example.shopping.discount.product.PromotionPricing;
...
public class Cart {
    ...
    public void add(Item itemToBuy, int howMany) {
    	...
    	// Apply 30% discount in case of gold card
    	if (user.getType() == UserType.GOLD) {
    		item = new PromotionPricing(itemToBuy, 30);
    	}
    	// Apply 20% discount in case of silver card
    	else if (user.getType() == UserType.SILVER) {
    		item = new PromotionPricing(itemToBuy, 20);
    	}
